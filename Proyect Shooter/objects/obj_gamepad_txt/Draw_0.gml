...
for (var i = 0; i <= limit; i++)
	{
		...
		draw_text_color(room_width/2, txt_y + (32 * i), string(button[i]), txt_col, txt_col, txt_col, txt_col, 1);
	}
...
//title
var gp = gamepad_get_description(global.controller);

draw_set_font(fnt_3);
draw_set_halign(fa_center);

if (global.controller > -1) && !(global.gicGamepadOnly == "True")
	{
		//draw_text(room_width/2, 10, string(global.txt13) + " - " + string(gp));
		draw_text(room_width/2, 10, string(global.txt13) + "\n¨" + string(gp) + "¨");
	}
else
	{
		draw_text(room_width/2, 10, string(global.txt13));
